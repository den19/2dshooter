...
public class Controller : MonoBehaviour
{
    ...
    public UnityEngine.InputSystem.InputAction moveAction;
    [Tooltip("The input action(s) that map to where the controller looks")]
    public UnityEngine.InputSystem.InputAction lookAction;
    ...
    public float baseMoveSpeed = 8;                  // Максимальная скорость движения
    public float smoothAccelerationTime = 1f;       // Время достижения полной скорости
    public float smoothDecelerationTime = 0.1f;       // Время остановки при снятии касания
    public float boundaryMargin = 0.1f;               // Безопасное расстояние от краёв экрана
    public LayerMask groundLayer;                     // Маска слоя для ограничения движения

    private Vector2 touchPosition;                    // Позиция касания на экране
    private bool isDragging;                          // Флаг активного касания
    private Vector3 velocity;                         // Вектор скорости для SmoothDamp


    public float smoothingFactor = 2.5f;      // Коэффициент сглаживания (чем выше, тем быстрее выравнивание)
    public float minRotationLimit = -45f;   // Мин. ограничение угла
    public float maxRotationLimit = 45f;    // Макс. ограничение угла
    ...
    void HandleMobileInput()
    {
        ...
        {
            Touch touch = Input.touches[0]; // Первое касание
        ...
        if (Input.touchCount >= 2)
        {
            ...
            float prevAngle = AngleBetweenPoints(touchZeroPrevPos, touchOnePrevPos);
            float currentAngle = AngleBetweenPoints(touchZero.position, touchOne.position);

            float angleDifference = currentAngle - prevAngle;

            // Меняем целевой угол относительно разности
            targetRotation += angleDifference * rotationSpeed;

            // Ограничиваем целевой угол
            targetRotation = Mathf.Clamp(targetRotation, minRotationLimit, maxRotationLimit);
